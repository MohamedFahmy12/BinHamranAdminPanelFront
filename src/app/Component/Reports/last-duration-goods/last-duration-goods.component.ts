...
@Component({
  selector: 'app-last-duration-goods',
  templateUrl: './last-duration-goods.component.html',
  styleUrls: ['./last-duration-goods.component.css']
})
export class LastDurationGoodsComponent implements OnInit {
  ...
  private router: Router, private datehelp: DateHelperService, private translate: TranslateService) { }
    ...
    ViewReport() {
      ...
      this.ReportSer.LastDurationGoods(this.sDate,this.ComIDS, this.dbIds,this.currentLocation).subscribe(
        (data: Response) => {
          debugger;
          this.result = data;
